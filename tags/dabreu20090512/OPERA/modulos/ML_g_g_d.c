...
void Covars_g_g_d(int n,double *x,double *errx,double mean,double sigma,double covar[2][2]);
...
int  ML_g_g_d(int n,double *x,double *errx,double *mean,double *sigma,double *errmean,double *errsigma,double *covarmeansigma) {
  ...
  m=StErrWeightMedia_d(n,x,errx,&dnul); 
  snew=dnul;
  meanerrx=StMedia_d(n,errx,&stderrx);
  ...
  do {
    ...
    /* Aqui pongo varias formulas, todas válidas, pero la mejor es la de tmp7,8,9,10 */
...
/*     if(snew>meanerrx/5.)    sigma_method=1; */
  ...
  *sigma=snew;
  ...
  Covars_g_g_d(n,x,errx,m,snew,covar);
