...

{
  ...
  float aa,bb,cc,dd,ee,q,xx0,yy0,tmp,z1,z2;
...
/*  --------- */
  ...
  if(!TestDiv0(cc*cc,4.0*aa,&z1,1E-10) ||
     !TestDiv0(dd*dd,4.0*bb,&z2,1E-10) ) return(0);
  z1 += z2-ee;
  if(!TestDiv0(z1,aa*bb,&q,1E-10)) return(0);
  if( q <= 0) return(0);

  if(!TestDiv0(-cc,2.0*aa,&xx0,1E-10)) return(0);
  if(!TestDiv0(-dd,2.0*bb,&yy0,1E-10)) return(0);

  *x0 = xx0*cos(*t)-yy0*sin(*t);
  *y0 = xx0*sin(*t)+yy0*cos(*t);

  if(DEBUG) printf(" xx0 %f yy0 %f\n",xx0,yy0);
