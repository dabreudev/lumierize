...
/* No esta del todo incluido lo del SKY!!!!*/
/* Ni lo del instsetup */
...
int interact=0;
int amoegauss=0;
int asstatus=1,mcstatus=1,mlstatus=1,sestatus=1,specstatus=1,skystatus=1;
...
/*data matrix */
float *flux,*errflux,*magni,*xobj,*yobj,*sky,*sig;
int *ilog;
int ndat;
...
void funmaglim(float x,float *p,float *y,float *dyda,int n);
...
void LoadParam_file(void) {
  ...
  ffgky(parfile,TSTRING,"SETUP",instsetup,comment,&status);
  fits_report_error(stderr,status);
...
void ReadCat(void) {
  ...
  if((sky =malloc(ndat*sizeof(float)))==NULL) printf("I cannot dimension xobj  of %d elements \n",ndat);
  if((sig =malloc(ndat*sizeof(float)))==NULL) printf("I cannot dimension yobj  of %d elements \n",ndat);
  ...
  ReadNumcol(filecross,skycol ,sky ,ilog,&ndat);
  ReadNumcol(filecross,sigcol ,sig ,ilog,&ndat);
...
/*    float *fsel; */
  ...
  covarpar=matrix_f(3,3);
  covarpar_d=matrix_d(3,3);
  mcal=vector_f(ndat);
...
/*   mminimum=mmin; */
  ...
  mmin=mminimum-(mmaximum-mminimum)*0.1;
...
/*    par[0]=0.5 ; sigpar[0]=0.1 ;  */
/*    par[1]=(mmaximum+mmode)/2.;sigpar[1]=1.; */
/*    par[2]=(mmaximum-mmode)/2.;sigpar[2]=par[2]/3.; */
/*    amoegauss=0; */
  

/*    iter=Amoeba(nbin,x,y,3,par,sigpar,FTOL,1000); */
...
/*   printf(" 222 666 ka\n"); */
  ...
  par[0]=log((float)twothirdmax/(float)thirdmax)/(mtwothirdmax-mthirdmax);
/*   par[0]=0.5; */
  par[1]=(mmode);
  if(DEBUG) printf(" mhm %f mmo %f ma %d hal %d\n",mhalfmaxd,mmode,maxhist,halfmaxd);  
  if(halfmaxd==maxhist) par[2]=(mhalfmaxd-mmode)/log(2);
  else                 par[2]=(mhalfmaxd-mmode)/log(2*(float)maxhist/(float)halfmaxd-1);
  if(par[2]>1/par[0]) par[2]=0.8/par[0];
/*   printf(" TENTA %f %f %f\n",par[0],par[1],par[2]); */

/*   par[1]=par[1]+1*Gasdev();  */
/*   par[2]=par[2]+0.15*Gasdev();   */
/*   par[0]=par[0]+0.3*Gasdev();  */
/*   par[0]=1.5; */
/*   par[1]=21.3; */
/*   par[2]=0.34; */

  par_d[0]=par[0];par_d[1]=par[1];par_d[2]=par[2];
...
/*   printf(" iter -1 %f %f %f \n",par[0],par[1],par[2]); */
  ...
  sumy=sumf=0.0;
  for(i=0;i<nbin;i++) {
    sumf+=DerPowLaw(x[i],1.,0.,par[0])*Fermi(x[i],par[1],par[2]);
    sumy+=y[i]; 
    /*       printf(" sumf %g sumy %g\n",sumf,sumy); */
  }  
  fact=sumy/sumf;
  /*   par[3]=log(fact); */
  /*     printf(" fact %g\n",fact); */
...
/*   free(x_d); */
/*   printf(" Pasa fre\n"); */
  iter=Mrq_d(x_d,y_d,sigy_d,nbin,par_d,ipar,3,covarpar_d,&chisq_d,funmaglim_d);
...
/*   goto salta;  */
  ...
  sumy=sumf=0.0;
  for(i=0;i<nbin;i++) {
    sumf+=DerPowLaw((float)x_d[i],1.,0.,(float)par_d[0])*Fermi(x_d[i],par_d[1],par_d[2]);
    sumy+=y_d[i]; 
  }  
  fact=sumy/sumf;
  if(DEBUG) printf(" gact %f sumy %f sumf %f\n",fact,sumy,sumf);
  mmax=par_d[1]+par_d[2]*log(1/0.0017-1);
  if(fact==0) mmin=mminimum-(mmaximum-mminimum)*0.1;
  else     mmin=log(maxhist/10./fact)/par_d[0];

  if(DEBUG) printf(" par0 %f\n",par[0]);
  if(DEBUG) printf(" pard0 %f\n",par_d[0]);
  if(DEBUG) printf(" Aqui mmin %g maxhist %d fact %g\n",mmin,maxhist,fact);
  if(mmax>mmaximum+(mmaximum-mminimum)*0.2) mmax=mmaximum+(mmaximum-mminimum)*0.2;
  if(mmin<mminimum-(mmaximum-mminimum)*0.1) mmin=mminimum-(mmaximum-mminimum)*0.1;
  free(histo);
  histo=StHisto2(nhist,mcal,nbin,&mmin,&mmax);
  deltabin=(mmax-mmin)/nbin;
  maxhist=0;
...
/*    printf(" DEsp allo\n");  */
  ...
  par_d[0]=par_d[0]; 
  if(par_d[2]>1/par_d[0]) par_d[2]=0.8/par_d[0]; 
  printf(" Calling MRQ...\n");
/*   printf(" iter INI %f %f %f \n",par_d[0],par_d[1],par_d[2]);  */
  cpgswin(mmin,mmax,0.,maxhist*1.2);
  if(DEBUG) {
...
/*  salta: */
  ...
  covarpar[2][0]=covarpar_d[2][0];
  covarpar[0][1]=covarpar_d[0][1];
  covarpar[1][1]=covarpar_d[1][1];
  covarpar[2][1]=covarpar_d[2][1];
  covarpar[0][2]=covarpar_d[0][2];
  covarpar[1][2]=covarpar_d[1][2];
  covarpar[2][2]=covarpar_d[2][2];
  chisq=chisq_d;
  printf(" DONE\n");
  
/*   printf(" %d iter MRQ: solpar:",iter);  */
/*   for(i=0;i<3;i++) printf(" par[%d] %11g",i,par[i]);  */
/*   printf(" chi %10g\n",chisq);  */
/*   printf("\n");  */
/*   printf(" %d iter MRQ: sig:",iter);  */
/*   for(i=0;i<3;i++) printf(" sig[%d] %11g",i,sqrt(covarpar[i+i*3]));  */
/*   printf(" \n");  */
/*   printf(" %d iter MRQ: covars:",iter);  */
/*   for(i=0;i<3;i++) */
/*     for(j=0;j<3;j++)  printf(" covar[%d %d] %11g",i,j,covarpar[i+j*3]);  */
/*   printf(" \n"); */
  

  mfermicut=par_d[1];deltafermi=par_d[2];gammapowerlaw=par_d[0];
  errmfermicut=sqrt(covarpar_d[1][1]);errdeltafermi=sqrt(covarpar_d[2][2]);errgammapowerlaw=sqrt(covarpar_d[0][0]);
  ...
  for(i=0;i<nbin;i++ ){
    ...
    cpgdraw(xdum,Fermi(xdum,(double)par_d[1],(double)par_d[2]));
  }
  sumy=sumf=0.0;
  for(i=0;i<nbin;i++) {
    sumf+=DerPowLaw((float)x_d[i],1.,0.,(float)par_d[0])*Fermi(x_d[i],par_d[1],par_d[2]);
...
/*   printf("  fact %g  sumy %g sumf %g \n",fact,sumy,sumf); */
  ...
  for(i=0;i<nbin;i++ ){
    xdum=i*(mmax-mmin)/nbin+mmin;
    cpgdraw(xdum,DerPowLaw(xdum,fact,0.,(float)par_d[0]));
/*      cpgdraw(xdum,DerPowLaw(xdum,par[0],par[2],par[1])*Fselerf(xdum,par[3],par[4]));  */
  }
  ...
  for(i=0;i<nbin;i++ ){
    xdum=i*(mmax-mmin)/nbin+mmin;
    cpgdraw(xdum,DerPowLaw(xdum,fact,0.,(float)par_d[0])*Fermi(xdum,par_d[1],par_d[2]));
  }
/*    sumy=sumf=0.0; */
/*    for(i=0;i<nbin;i++) { */
/*      sumf+=DerPowLaw(x[i],1.,0.,sigpar[0])*Fermi(x[i],sigpar[1],sigpar[2]); */
/*      sumy+=y[i];  */
/*    }   */
/*    fact=sumy/sumf; */
/*    cpgsci(5); */
...
/*    for(i=0;i<nbin;i++ ){ */
/*      xdum=i*(mmax-mmin)/nbin+mmin; */
/*      cpgdraw(xdum,DerPowLaw(xdum,fact,0.,sigpar[0])*Fermi(xdum,sigpar[1],sigpar[2])); */
/*    } */
  
/*   printf("  Antes de freear\n"); */
...
/*     printf("VREADC\n"); */
  ...
  if(specstatus)
    fprintf(fr,"%-51s  %-51s  ",specfile,respfile);
  else 
...
void funmaglim(float x,float *p,float *y,float *dyda,int n) {
  ...
  if(p[1]<19) p[1]=(mmaximum+mmode)/2.; 
  if(p[1]<xglob[0]) p[1]=(mmaximum+mmode)/2.; 
  if(p[1]>xglob[nbin-1]) p[1]=(mmaximum+mmode)/2.; 
  if(p[2]<0) p[2]=(mmaximum-mmode)/2.; 
/*   if(p[0]>1/p[2]) p[0]=0.8*p[2];  */
  

  MinMax(nbin,xglob,&xmin,&xmax);

  for(i=0;i<nbin;i++) {
    sumf+=DerPowLaw(xglob[i],1.,0.,p[0])*Fermi(xglob[i],p[1],p[2]);
    sumy+=yglob[i]; 
  }
  ...
  cpgmove(xmin,0.);
  for(i=0;i<nbin;i++)   cpgdraw(xglob[i],DerPowLaw(xglob[i],fact,0.,p[0])*Fermi(xglob[i],p[1],p[2])); 
/*   for(i=0;i<nbin;i++)   cpgdraw(xglob[i],DerPowLaw(xglob[i],fact,p[3],p[0])*Fermi(xglob[i],p[1],p[2])); */

  *y=DerPowLaw(x,fact,0.,p[0])*Fermi(x,p[1],p[2]); 
/*   *y=DerPowLaw(x,fact,p[3],p[0])*Fermi(x,p[1],p[2]); */
  dyda[0]=*y*x;
  dyda[1]=*y*Fermi(x,p[1],p[2])*(exp((x-p[1])/p[2]))/p[2];
  dyda[2]=+dyda[1]/p[2]*(x-p[1]);
  dyda[3]=*y;
  printf(" y %f p1 %f p2 %f p3 %f \n",*y,p[0],p[1],p[2]);
...
void funmaglim_d(double x,double *p,double *y,double *dyda,int n) {
  ...
  if(DEBUG)  printf(" Antes p1 %f p2 %f p3 %f delta %f\n",p[0],p[1],p[2],deltabin); 

/*   if(p[1]<19) { */
/*     printf(" WA1\n"); */
/*     p[1]=(mmaximum+mmode)/2.;  */
/*   } */
  if(p[1]<xglob_d[0]) {
/*     printf(" WA2\n"); */
    p[1]=mmode; 
  }
  if(p[1]>xglob_d[nbin-1]) {
/*     printf(" WA3\n"); */
    p[1]=mmode; 
  }
   if(p[2]<deltabin/5.) { 
...
/*     printf(" np0 %f\n",p[0]); */
  ...
  sumy=0;
  for(i=0;i<nbin;i++) {
/*     sumf+=DerPowLaw(xglob[i],1.,0.,(float)p[0])*Fermi(xglob[i],(float)p[1],(float)p[2]); */
    sumf+=exp(xglob_d[i]*p[0])*Fermi(xglob_d[i],p[1],p[2]);
    sumy+=yglob_d[i]; 
  }
...
/*   cpgmove(xmin,0.); */
/*   for(i=0;i<nbin;i++)   cpgdraw((float)xglob_d[i],DerPowLaw((float)xglob_d[i],fact,0.,(float)p[0])*Fermi((float)xglob_d[i],(float)p[1],(float)p[2]));  */

/*   *y=DerPowLaw(x,fact,0.,p[0])*Fermi(x,(float)p[1],(float)p[2]);  */
  *y=fact*exp(x*p[0])*Fermi(x,p[1],p[2]); 
  dyda[0]=*y*x;
  if(DEBUG) printf(" f %g e %g\n",Fermi(x,p[1],p[2]),(exp((x-p[1])/p[2])));
  dyda[1]=*y*Fermi(x,p[1],p[2])*(exp((x-p[1])/p[2]))/p[2];
  dyda[2]=+dyda[1]/p[2]*(x-p[1]);
/*   dyda[3]=*y; */
  if(DEBUG) printf("Despues  y %f p1 %f p2 %f p3 %f \n",*y,p[0],p[1],p[2]); 
...
int MeanSky(void) {
  ...
  float *truesky,*truesig;
  int n;
  int i;
  float firstsky,thirdsky;
  float firstsig,thirdsig;

  truesky=vector_f(ndat);
  truesig=vector_f(ndat);
  n=0;
  for(i=0;i<ndat;i++) {
     if(ilog[i]) {
        truesky[n]=sky[i];
  ...
  Quartil(n,truesky,&firstsky,&meansky,&thirdsky);
  printf(" Sky quartil f %f me %f t %f\n",firstsky,meansky,meansky);
  Quartil(n,truesig,&firstsig,&meansig,&thirdsig);
  printf(" Sig quartil f %f me %f t %f\n",firstsig,meansig,meansig);
