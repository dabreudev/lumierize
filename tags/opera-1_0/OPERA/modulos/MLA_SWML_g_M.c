...
double Amoe_Funk_SWML_g_M_main(int n, double *x, double *y, double *p);
double Amoe_Funk_SWML_g_M_norm(int n, double *x, double *y, double *p);
double Amoe_Funk_SWML_g_M_prior(int n, double *x, double *y, double *p);
double Amoe_Funk_SWML_g_M_conf(int n, double *x, double *y, double *p);
double Funk2_int_SWML_g_M(double x);
double Funk1_norm_SWML_g_M(double x);
void   EmpiricalCovars_SWML_g_M(int n,double *magn,double *errmag,double *z, double *errz,double *par, double *sigpar,double mlim, struct cosmo_param cosmo,struct Steplf_M *lf);

void   ComputeNorma_SWML_g_M(int n, double mlim, double strrad, double zlow, double zup, struct Steplf_M *lf);
...
int  MLA_SWML_g_M(int n,double *magn,double *errmag,double *z, double *errz,double mlim, double strrad, double zlow, double zup, struct cosmo_param cosmo,struct Steplf_M *lf) {
...
/*   par[3]=0.3;sigpar[3]=0.2; */
  ...
  iter_amo=Amoeba_d(n,magn,z,lf->nbin,par,sigpar,FTOL,MAXITER,Amoe_Funk_SWML_g_M_main);


  MLmax=Amoe_Funk_SWML_g_M_main(n,magn,z,par);
  ...
  if(TRYEMPIRICAL) {
    ...
    conflim=exp(-.5/10.);
    EmpiricalCovars_SWML_g_M(n,magn,errmag,z,errz,par,sigpar,mlim,cosmo,lf);  
    if(DEBUG) printf(" sale \n");
  }
  if(CONTOURPLOT) {
/*     nemp_f++; */
/*     conflim=exp(-.5*16.);     */
/*     EmpiricalCovars_SWML_g_M(n,x,errx,k,xk,Pk,sigpar,covar);  */
/*     nemp_f++; */
/*     conflim=exp(-.5*9.);     */
/*     EmpiricalCovars_SWML_g_M(n,x,errx,k,xk,Pk,sigpar,covar);  */
/*     nemp_f++; */
...
/*   printf(" MLM %g conf %f   %f %f\n",MLmax,conflim,fabs(Amoe_Funk_SWML_g_M_main(n,x,y,p)-(MLmax-log(conflim))),Amoe_Funk_SWML_g_M_main(n,x,y,p)); */
   
  return(fabs(Amoe_Funk_SWML_g_M_main(n,x,y,p)-(MLmax-log(conflim)))); 
}
...
/*     printf(" Por %d %d \n",i,nconfl); */
    ...
    iter_c=Amoeba_d(n,magn,z,lf->nbin,parconf,sigparconf,FTOL2,MAXITER2,Amoe_Funk_SWML_g_M_conf);
    ...
    if(iter_c==0 && Amoe_Funk_SWML_g_M_conf(n,magn,z,parconf)>FTOL2 ) {
      printf("\b.\b");
      i--;
      fflush(NULL);
    }
