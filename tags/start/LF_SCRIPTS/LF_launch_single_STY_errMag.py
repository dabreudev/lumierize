...
# TITLE: Script para generar calcular la LF con STY_errMag
# CREATED: 20080326 dabreu
# Este script lanza el cálculo de la LF con STY_errMag
#
# MODIFIED: 20080425 dabreu
# Lockfiles structure modified.
# 
# Uso: LF_launch_single_STY_errMag.py -p simulationParameterFile numberOfIter
#

"""Script para calcualr la LF con STY_errMag"""
...
def main():
    ...
    def signal_handler(signal, frame):
        ...
        output.write("Recibí un SIGTERM (habló launch_single_STY_errMag)\n")
        try:
    ...
    for numObjects in nobjects:
        ...
        lfDir=simulationsDir+'/ComputedLF/STY_errMag/'+simulationName
        if not(os.access(lfDir,os.F_OK)):
            os.makedirs(lfDir)
            if verbose: output.write("Creating "+lfDir+"\n")
        computedSTY_errMagFile=lfDir+str(simulationNumber)+".lf"

        # done y lock files
        doneDir=simulationsDir+"/LockFiles/STY_errMag/"+simulationName
        ...
        doneSTY_errMagFileName=doneDir+str(simulationNumber)+".done"
        lockSTY_errMagFileName=doneDir+str(simulationNumber)+".lock"

        comando ="LF_computeSTY_errMag.csh "+simulatedCatalogFile+" "
        comando+=str(mlim)
        comando+=" "+str(zlow)+" "+str(zup)+" "+str(area)
	comando+=" "+computedSTY_errMagFile

        doneFile=0
        doneFile=os.access(doneCatFileName,os.F_OK)

        lockFileName=lockSTY_errMagFileName # para el capturador de sigterm
        global PID
        PID=False

        if doneFile:
            # mandamos el catálogo
            estatusSTY_errMag=secureCompute('STY_errMag',doneSTY_errMagFileName,lockSTY_errMagFileName,comando,computedSTY_errMagFile)
            if estatusSTY_errMag==1:
                if verbose: output.write("Problemas con STY_errMag\n")
	        continue # seguimos con otro
            else:
                if verbose: output.write("Todo correcto con STY_errMag\n")
	 
            if verbose: output.write("Ya está hecho STY_errMag para nobjects ="+str(numObjects)+"\n")
            continue
        else:
            if verbose: output.write("No se puede hacer STY_errMag porque no está el catálogo.\n")

    if verbose: output.write("Iter ="+str(simulationNumber)+"\n")

    # Cuando termina el bucle en número de galaxias
    if verbose: output.write("\nLF_launch_single_STY_errMag.py termina aquí\n")
    if verbose: output.write("\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n")
    output.close()
    sys.exit(0)
