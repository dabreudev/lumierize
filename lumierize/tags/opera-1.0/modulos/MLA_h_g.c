...
float Amoe_Funk_prior(int n, float *x, float *y, float *p);
float Amoe_Funk_conf(int n, float *x, float *y, float *p);
void   TeorCovars_h_g(int n,float *x,float *errx,int k,float *xk,float *Pk,float **covar);
void   EmpiricalCovars_h_g(int n,float *x,float *errx,int k,float *xk,float *Pk,float *sigPk,float **covar);
float  Forma(int ndim, float **x, float **C, int ii);
...
int nconfl;
float conflim;
/* float ML[5*MAXTRIES*MAXITER]; */
float MLmax;
float *pcut;

int nemp=2;

int  MLA_h_g_f(int n,float *x,float *errx, int k, float *xk, float *Pk, float **covPk) {
  ...
  while(iter_amo>= MAXITER) {
    if((histo=StHisto2(n,x,k,xk,xk+k))==NULL) { printf("I cannot dimension histo of %d elements \n",k);exit(1);}

    for(j=0;j<kpar;j++) {
      if(histo[j]==0)  par[j]=   1.           /(xk[j+1]-xk[j]);
      else             par[j]=(float)histo[j]/(xk[j+1]-xk[j]);
    }
    norm=0;
    ...
    iter_amo=Amoeba(n,x,y,k,par,sigpar,FTOL,MAXITER,Amoe_Funk_main); 
...
/*     free(histo);  */
  ...
  MLmax=Amoe_Funk_main(n,x,y,par);
/*   if(DEBUG) printf(" ML max %.10f\n",MLmax); */
/*   if(DEBUG) printf(" FINAL par0 %.15g par1 %.15g par2 %.15g\n",par[0],par[1],par[2]); */
/*   iter_amo=Amoeba(n,x,y,k,par,sigpar,FTOL,MAXITER,Amoe_Funk_prior);  */
/*   MLmax=Amoe_Funk_main(n,x,y,par); */
/*   if(DEBUG) printf(" ML max %.10f\n",MLmax); */
/*   if(DEBUG) printf(" FINAL par0 %.15g par1 %.15g par2 %.15g\n",par[0],par[1],par[2]); */
  ...
  TeorCovars_h_g(n,x,errx,k,xk,Pk,covar);  
...
/*   i=readi(i); */
  ...
  if(DEBUGPLOT) {
    cpgpage();  
    cpgswin(Pk[0]-0.02,Pk[0]+0.02,Pk[2]-0.02,Pk[2]+0.02);
    cpgbox("BCTNS",0,0,"BCTNS",0,0);
  }

  if(TRYEMPIRICAL) {
    conflim=exp(-.5/10.);
    EmpiricalCovars_h_g(n,x,errx,k,xk,Pk,sigpar,covar); 
  }
  if(CONTOURPLOT) {
    nemp++;
    conflim=exp(-.5*16.);    
...
void TeorCovars_h_g(int n,float *x,float *errx,int k,float *xk,float *Pk,float **covar) {
  ...
  free_matrix_f(hessian,k+1,k+1);
  free_matrix_f(b,k+1,1);
}
...
/*   float xtmp,ytmp; */
  ...
  bb=matrix_f(kpar-1,1);
  y=vector_f(n);
  parconf=vector_f(k);
  sigparconf=vector_f(k);

  parelip=matrix_f(k  ,nconflini);
  pareA  =matrix_f(k-1,nconflini);
  pareB  =matrix_f(k-1,nconflini);
  pareC  =matrix_f(k-1,nconflini);

  invcovar=matrix_f(kpar  ,kpar  );
  invcovA =matrix_f(kpar-1,kpar-1);
  invcovB =matrix_f(kpar-1,kpar-1);
  invcovC =matrix_f(kpar-1,kpar-1);
  covA    =matrix_f(kpar-1,kpar-1);
  covB    =matrix_f(kpar-1,kpar-1);
  covC    =matrix_f(kpar-1,kpar-1);
  distmax=vector_f(2);
...
/*   conflim=exp(-.5/100.);    */
  ...
  for(i=0;i<nconfl;i++) {
    ...
    if(i>(int)(nconfl/2.)) {
      for(j=0;j<kpar;j++) {
	parconf[j]=Pk[j]-((parelip[(int)(i-nconfl/2.)+1])[j]-Pk[j]);
	sigparconf[j]=((parelip[(int)(i-nconfl/2.)+1])[j]-Pk[j])/2.; 
      }
      if(DEBUG3) {
	cpgsci(3);
    ...
    iter_c=0;
    iter_c=Amoeba(n,x,y,k,parconf,sigparconf,FTOL2,MAXITER2,Amoe_Funk_conf);

    if(DEBUG3) printf(" PARCONF %g %g iter_c %d AMF %g i %d\n",parconf[0],parconf[1],iter_c,Amoe_Funk_conf(n,x,y,parconf),i);

    for(j=0;j<kpar;j++) {
      (parelip[i])[j]=parconf[j];
/*       phi=M_PI*Gasdev(); */
/*       teta=M_PI*Gasdev(); */
/*       (parelip[i])[0]=Pk[0]+1*cos(phi)*sin(teta); */
    ...
    if(DEBUG3) {
      ...
      for(j=0;j<i;j++)  {
	printf(" ## ");
	for(l=0;l<kpar;l++)    printf(" %g ",(parelip[j])[l]);
      }
      printf("\n");
    }
      
    if(iter_c==0 && Amoe_Funk_conf(n,x,y,parconf)>FTOL2 ) i--;
  }
  ...
  if(DEBUG) {
    ...
    for(j=0;j<nconfl;j++)  {
      ...
      printf(" \n");
      cpgsci(7);
      cpgpt1((parelip[j])[0],(parelip[j])[1],6);
    }
    printf("\n");
  }
/*   i=readi(i); */
  ...
  /* Supongo que el centro de la elipse es el valor que maximiza ML */
   ...
   for(i=0;i<nconfl;i++)    for(j=0;j<kpar;j++)       (parelip[i])[j]-=Pk[j];


  if(DEBUGPLOT) {
    for(i=0;i<nconfl;i++) {
      if(DEBUG3) printf(" PARCONFREAL %g %g\n",(parelip[i])[0],(parelip[i])[1]);
      cpgsci(nemp);
      cpgpt1((parelip[i])[0]+Pk[0],(parelip[i])[2]+Pk[2],1);
    }
  }
...
/*   kpar=readi(kpar); */
  ...
  for(i=0;i<nconfl;i++) {
    for(j=0;j<kpar-1;j++) {
      pareA[i][j]=parelip[i][j];
    }
  }
  MCElipN(nconfl,kpar-1,pareA,invcovA);
  for(i=0;i<nconfl;i++) {
    for(j=0;j<kpar-1;j++) {
      pareB[i][j]=parelip[i][j+1];
    }
  }
  MCElipN(nconfl,kpar-1,pareB,invcovB);
  for(i=0;i<nconfl;i++) {
    for(j=0;j<kpar-2;j++) {
      pareC[i][j]=parelip[i][j];
    }
    pareC[i][kpar-2]=parelip[i][kpar-1];
  }
  MCElipN(nconfl,kpar-1,pareC,invcovC);
  ...
  gaussj(covA,kpar-1,bb,1);
  gaussj(covB,kpar-1,bb,1);
  gaussj(covC,kpar-1,bb,1);
  if(DEBUG2) {
    printf(" COVA\n");
    for(i=0;i<k-1;i++) {
      for(j=0;j<k-1;j++) {
  ...
  free_matrix_f(bb,kpar-1,1);
  free(y);
  free(parconf);
  free(sigparconf);

  free_matrix_f(parelip,kpar  ,nconflini);
  free_matrix_f(pareA  ,kpar-1,nconflini);
  free_matrix_f(pareB  ,kpar-1,nconflini);
  free_matrix_f(pareC  ,kpar-1,nconflini);

  free_matrix_f(invcovar,kpar  ,kpar);
  free_matrix_f(invcovA ,kpar-1,kpar-1);
  free_matrix_f(invcovB ,kpar-1,kpar-1);
  free_matrix_f(invcovC ,kpar-1,kpar-1);
