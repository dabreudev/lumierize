...
double Amoe_Funk_STY_s_p_f_PO_main(int n, double *x, double *y, double *p);
double Amoe_Funk_STY_s_p_f_PO_norm(int n, double *x, double *y, double *p);
double Amoe_Funk_STY_s_p_f_PO_prior(int n, double *x, double *y, double *p);
double Amoe_Funk_STY_s_p_f_PO_conf(int n, double *x, double *y, double *p);
double Funk2_int_STY_s_p_f_PO(double x);
double Funk1_norm_STY_s_p_f_PO(double x);
...
double *ew_STY_s_p_f_PO;
int    *isur_STY_s_p_f_PO;
struct cosmo_param *co_STY_s_p_f_PO;
double ewl_STY_s_p_f_PO;
struct Histdist *ewd_STY_s_p_f_PO;
struct poselfunc *fsel_STY_s_p_f_PO;
...
int  MLA_STY_s_p_f_PO  (int n,double *magn,double *ew,double *z,int *isurvey,char photband[51], float gamma_f, float delta, float Kcoc, struct poselfunc fsel, struct SurveyDB sdb,  double ewlim, struct Histdist ewd, struct cosmo_param cosmo, struct Schlf_L *lf) {
  ...
  ndata=n;
  co_STY_s_p_f_PO=&cosmo;
  ewd_STY_s_p_f_PO=&ewd;
  fsel_STY_s_p_f_PO=&fsel;
  sdb_STY_s_p_f_PO=&sdb;
  ewl_STY_s_p_f_PO=ewlim;
  ew_STY_s_p_f_PO=ew;
...
/*     sigpar[1]=1; */
/*     sigpar[2]= */
    par[0]=-1.1; sigpar[0]=0.1;
    par[1]=35.3; sigpar[1]=0.5;
    ...
    iter_amo=Amoeba_d(n,magn,z,3,par,sigpar,FTOL,MAXITER,Amoe_Funk_STY_s_p_f_PO_main);  
  ...
  MLmax=Amoe_Funk_STY_s_p_f_PO_main(n,magn,z,par);
...
/*   double Lup; */
  ...
  normaewd=0;
  for(i=0;i<ewd_STY_s_p_f_PO->k;i++) {
    if(ewl_STY_s_p_f_PO>ewd_STY_s_p_f_PO->xk[i+1]);
    else if(ewl_STY_s_p_f_PO>ewd_STY_s_p_f_PO->xk[i])  normaewd+=ewd_STY_s_p_f_PO->Pk[i]*(ewd_STY_s_p_f_PO->xk[i+1]-ewl_STY_s_p_f_PO);
    else normaewd+=ewd_STY_s_p_f_PO->Pk[i]*(ewd_STY_s_p_f_PO->xk[i+1]-ewd_STY_s_p_f_PO->xk[i]);
  }


  logL=0.;
  if(nEW<2*(ewd_STY_s_p_f_PO->k)) nEW=2*(ewd_STY_s_p_f_PO->k);

  intsup=incom(1+lfamo.alfa,200.);
  probabajo=Int_sch_s_f_PO(lfamo, photband_STY_s_p_f_PO,gamma_STY_s_p_f_PO, delta_STY_s_p_f_PO,Kcoc_STY_s_p_f_PO,*fsel_STY_s_p_f_PO,*sdb_STY_s_p_f_PO,ewl_STY_s_p_f_PO,*ewd_STY_s_p_f_PO,*co_STY_s_p_f_PO); 
  if(probabajo==0) {
    printf(" MLA_STY_s_p_f_PO: probaabajo=0. Exiting\n");
    exit(1);
  }
  for(i=0;i<ndata;i++) {
    isurvey=isur_STY_s_p_f_PO[i];
    z=y[i];
    magn=x[i];
    ew=ew_STY_s_p_f_PO[i];
    flux=Flux_ew_mag(ew,magn,photband_STY_s_p_f_PO,gamma_STY_s_p_f_PO,delta_STY_s_p_f_PO,Kcoc_STY_s_p_f_PO);
    Lumi=Lum(z,flux,*co_STY_s_p_f_PO);
    Mabs=-2.5*log10(Lumi);
    flim=Flux_ew_mag(ewl_STY_s_p_f_PO,fsel_STY_s_p_f_PO->magbin[fsel_STY_s_p_f_PO->nmag-1],photband_STY_s_p_f_PO,gamma_STY_s_p_f_PO,delta_STY_s_p_f_PO,Kcoc_STY_s_p_f_PO);
    Llow=Lum(z,flim,*co_STY_s_p_f_PO);
    xmin=Llow/lfamo.Lstar;
    if(DEBUG2) printf(" ew %f Hist %f\n",ew,Histfunc(ew,*ewd_STY_s_p_f_PO));
    dLdm=Lumi*0.4*log(10); /* Esto es exacto */
    isurvey=isur_STY_s_p_f_PO[i];
    pd=prob_poselfunc_scale(*fsel_STY_s_p_f_PO,sdb_STY_s_p_f_PO->si[isurvey],ew,z,magn);
    area=sdb_STY_s_p_f_PO->si[isurvey].xdim*sdb_STY_s_p_f_PO->si[isurvey].ydim;
    if(ew>ewd_STY_s_p_f_PO->xk[ewd_STY_s_p_f_PO->k])  {
      ewhis=Histfunc(ewd_STY_s_p_f_PO->xk[ewd_STY_s_p_f_PO->k],*ewd_STY_s_p_f_PO)/normaewd;
      printf(" Con esto ewhis %f\n",ewhis);
    }
    else ewhis=Histfunc(ew,*ewd_STY_s_p_f_PO)/normaewd;
    logprobarriba=log(area/4*M_PI)+Schechter_L(Lumi,lfamo)+log(ewhis)+log(dLdm)+log(dVdz(z,*co_STY_s_p_f_PO))+log(pd);
    if(DEBUG) printf(" area %f Sch %g His %f pd %f \n",area,Schechter_L(Lumi,lfamo),log(Histfunc(ew,*ewd_STY_s_p_f_PO)/normaewd),pd);
...
/*   Ntot=Int_sch_s_f_PO(lfamo, photband_STY_s_p_f_PO,gamma_STY_s_p_f_PO, delta_STY_s_p_f_PO,Kcoc_STY_s_p_f_PO,*fsel_STY_s_p_f_PO,*si_STY_s_p_f_PO,ewl_STY_s_p_f_PO,*ewd_STY_s_p_f_PO,*co_STY_s_p_f_PO); */
...
/*   printf(" MLM %g conf %f   %f %f\n",MLmax,conflim,fabs(Amoe_Funk_STY_s_p_f_PO_main(n,x,y,p)-(MLmax-log(conflim))),Amoe_Funk_STY_s_p_f_PO_main(n,x,y,p)); */
   
  return(fabs(Amoe_Funk_STY_s_p_f_PO_main(n,x,y,p)-(MLmax-log(conflim)))); 
}
...
/*   printf(" antes for \n"); */
  ...
  for(i=0;i<nconfl;i++) {
    ...
    iter_c=Amoeba_d(n,magn,z,3,parconf,sigparconf,FTOL2,MAXITER2,Amoe_Funk_STY_s_p_f_PO_conf);
    ...
    if(iter_c==0 && Amoe_Funk_STY_s_p_f_PO_conf(n,magn,z,parconf)>FTOL2 ) {
      printf("\b.\b");
      fflush(NULL);
      i--;
    }
