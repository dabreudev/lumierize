...
double Amoe_Funk_SWML_g_p_M_main(int n, double *x, double *y, double *p);
double Amoe_Funk_SWML_g_p_M_norm(int n, double *x, double *y, double *p);
double Amoe_Funk_SWML_g_p_M_prior(int n, double *x, double *y, double *p);
double Amoe_Funk_SWML_g_p_M_conf(int n, double *x, double *y, double *p);
double Funk2_int_SWML_g_p_M(double x);
double Funk1_norm_SWML_g_p_M(double x);
void   EmpiricalCovars_SWML_g_p_M(int n,double *magn,double *errmag,double *z, double *errz,double *par, double *sigpar,double mlim, struct cosmo_param cosmo,struct Steplf_M *lf);

void   ComputeNorma_SWML_g_p_M(int n, double mlim, double strrad, double zlow, double zup, struct Steplf_M *lf);
...
int  MLA_SWML_g_p_M(int n,double *magn,double *errmag,double *z, double *errz,double mlim, double strrad, double zlow, double zup, struct cosmo_param cosmo,struct Steplf_M *lf) {
  ...
  if(DEBUG) printf(" Estoy auiq dentro SWML_g_p_M\n");
  ...
  iter_m=0;
    ...
    for(i=0;i<n;i++) {
      ...
      if(DEBUG3) printf(" Entrada x %g errx %g\n",magn[i],errmag[i]);
    }
...
/*   par[3]=0.3;sigpar[3]=0.2; */
  ...
  iter_amo=Amoeba_d(n,magn,z,lf->nbin,par,sigpar,FTOL,MAXITER,Amoe_Funk_SWML_g_p_M_main);


  MLmax=Amoe_Funk_SWML_g_p_M_main(n,magn,z,par);
  ...
  if(TRYEMPIRICAL) {
    ...
    conflim=exp(-.5/10.);
    EmpiricalCovars_SWML_g_p_M(n,magn,errmag,z,errz,par,sigpar,mlim,cosmo,lf);  
...
/*     cpglab("P\\d1\\u","P\\d3\\u","Contornos de l�mites de confianza"); */
  ...
  if(DEBUG) {
    ...
    /* dabreu: no s� porqu� me da error al compilar */
    /* PrintStepLF_M(lf); */
    for(j=0;j<nbin;j++)  printf(" %d par %f sig %f hisot %d magni %f - %f\n",j,par[j],sigpar[j],histo[j],magbin[j],magbin[j+1]);
  }
/*   ComputeNorma_SWML_g_p_M(n, mlim,  strrad, zlow,  zup, lf); */
...
/*   printf(" MLM %g conf %f   %f %f\n",MLmax,conflim,fabs(Amoe_Funk_SWML_g_p_M_main(n,x,y,p)-(MLmax-log(conflim))),Amoe_Funk_SWML_g_p_M_main(n,x,y,p)); */
   
  return(fabs(Amoe_Funk_SWML_g_p_M_main(n,x,y,p)-(MLmax-log(conflim)))); 
}
...
/*     printf(" Por %d %d \n",i,nconfl); */
    ...
    iter_c=Amoeba_d(n,magn,z,lf->nbin,parconf,sigparconf,FTOL2,MAXITER2,Amoe_Funk_SWML_g_p_M_conf);
    if(DEBUG) printf(" %d CONF iter %d par0 %f    par1 %f\n", i,  iter_c ,parconf[0],   parconf[1]);
    for(j=0;j<lf->nbin;j++)  (parelip[j])[i]=parconf[j];
    if(iter_c==0 && Amoe_Funk_SWML_g_p_M_conf(n,magn,z,parconf)>FTOL2 ) {
      printf("\b.\b");
      i--;
      fflush(NULL);
    }
