...
double GA_Funk_STY_p_L_main(int n, double *x, double *y, double *p);
double GA_Funk_STY_p_L_norm(int n, double *x, double *y, double *p);
double GA_Funk_STY_p_L_prior(int n, double *x, double *y, double *p);
double GA_Funk_STY_p_L_conf(int n, double *x, double *y, double *p);
double Funk2_int_STY_p_L(double x);
double Funk1_norm_STY_p_L(double x);
void   GA_EmpiricalCovars_STY_p_L(int n,double *flux,double *z,double *par, double *sigpar,double flim, struct cosmo_param cosmo,struct Schlf_L *lf);

void   GA_ComputeNorma_STY_p_L(int n, double flim, double strrad, double zlow, double zup, struct Schlf_L *lf);
double *zsig;
...
int  MLGA_STY_p_L(int n,double *flux,double *z,double flim, double strrad, double zlow, double zup, struct cosmo_param cosmo,struct Schlf_L *lf) {
...
/*   PlotStepLF_L(lfvvmax); */
  ...
  /* dabreu: Error al compilar */
  /* if(DEBUG) PrintStepLF_L(lf); */
...
/*   cpgclos(); */
  ...
  while(gene_ga==0) { 
    ...
    gene_ga=gadriver_(n,flux,z,3,par,sigpar,FTOL,MAXITER,GA_Funk_STY_p_L_main);  
    if(DEBUG) printf(" Numero de generaciones%d\n",gene_ga);
    lf->alfa=par[0];
  ...
  MLmax=GA_Funk_STY_p_L_main(n,flux,z,par);
...
/*   double Lup; */
  ...
  lfga.alfa=p[0];
  lfga.Lstar=pow(10.,p[1]);
  lfga.phistar=exp(p[2]);

  lfga_M.alfa=p[0];
  lfga_M.Mstar=-2.5*p[1];
  lfga_M.phistar=exp(p[2]);

  logL=0.;

  intsup=incom(1+lfga.alfa,200.);
/*   printf(" intsup %f\n",intsup); */
  for(i=0;i<ndata;i++) {
    Lumi=Lum(y[i],x[i],*co);
    Mabs=-2.5*log10(Lumi);
    ...
    if(xmin>=200) {
	 ...
	 logL-= Schechter_L(Lumi,lfga) -log(intsch); 
	 Pero prefiero usar la funcion de Sch en magni */
/*       logL-= log(Schechter_M(Mabs,lfga_M)/intsch); */
      logL-= Schechter_L(Lumi,lfga)-log(intsch);
      if(DEBUG3) printf(" logL %f sch %g intsc %g intsup %g Lumi %g\n",logL,Schechter_L(Lumi,lfga),intsch,intsup,Lumi);
      /* Funciona perfectamente en magnitudes */
    }
/*     printf(" ndata %d  logL %g x %g  y %g Lumi %g Mabs %f alfa %f  Lstar %g Mstar %f Llow %g intsup %g  sch %g int %g xmin %g inco %g sch-int%g\n",i,logL,x[i],y[i],Lumi,Mabs,lfga.alfa,lfga.Lstar,lfga_M.Mstar,Llow,intsup,Schechter_L(Lumi,lfga),intsch,Llow/lfga.Lstar,incom(1+lfga.alfa,Llow/lfga.Lstar),Schechter_L(Lumi,lfga) -log(intsch)); */
/*     printf(" LF: lfga.Lstar %g lfga.phistar %f lf.alfa  %f\n",lfga.Lstar,lfga.phistar,lfga.alfa); */
  }
  /* Aqui viene la parte de la poissoniana de npob */
  Ntot=Int_sch_L(lfga,zlow_g,zup_g,fluxl,*co)*strrad_g/4./M_PI;
...
/*   printf(" Ntot %f\n",Ntot); */
  ...
  iter_m++;
  if(DEBUG2) printf(" Iter %d  logL %f par0 %g par1 %g par2 %g\n",iter_m,logL,p[0],p[1],p[2]);
  return(logL);
}
...
/*   printf(" MLM %g conf %f   %f %f\n",MLmax,conflim,fabs(GA_Funk_STY_p_L_main(n,x,y,p)-(MLmax-log(conflim))),GA_Funk_STY_p_L_main(n,x,y,p)); */
   
  return(fabs(GA_Funk_STY_p_L_main(n,x,y,p)-(MLmax-log(conflim)))); 
}
...
/*   printf(" antes for \n"); */
  ...
  for(i=0;i<nconfl;i++) {
    ...
    iter_c=gadriver_(n,flux,z,3,parconf,sigparconf,FTOL2,MAXITER2,GA_Funk_STY_p_L_conf);
    ...
    if(iter_c==0 && GA_Funk_STY_p_L_conf(n,flux,z,parconf)>FTOL2 ) {
      printf("\b.\b");
      fflush(NULL);
      i--;
    }
