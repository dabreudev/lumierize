...
/* Estructura para contener los par�metros */
...
double Amoe_Funk_STY_p_M_wC_main(int n, double *x, double *y, double *p)
{
  ...
  for(i=0;i<_ndata_STY_p_M_wC;i++)
  {
    ...
    /* debido a un underflow, tuvimos que poner este if
       ...
       para x > 0.25 y nos devolv�a un underflow cuando se cumpl�a la segunda
       condici�n -> los fuentes de gsl est�n en:
    ...
    */
    if(Llow/Lstar > 0.25 && (lfamo.alfa*log(Llow/Lstar) - Llow/Lstar) <= GSL_LOG_DBL_MIN)
    {
      ...
      if(DEBUG2) printf("log_gamma_int pod�a petar as� que GSL_LOG_DBL_MIN\n");
    }
    else
    {
    ...
    /* log(lfamo.phistar) + log_gamma_int -> integral de la funci�n de Schecter
    entre Llow e inf */
