...
/* Para compilar: */
/* 	cc  -I$PGPLOT_DIR -c $s2/Proced/C/modulos/cpgdodraw3d.c */
...
int fredgo_do(float *array,int * mn);
...
int cpgdodraw3d(double *arrayen, int kx, int ny, float size, float angle)
{
  ...
  int my;
/*   Subroutine */
/*   extern   int pgbbuf_(), fredgo_do_(), pgebuf_(); */
  float deltav, deltay;
/*   Subroutine */
/*   extern   int pgdraw_(), pgmove_(); */
  array=malloc(kx * ny*sizeof(float));
  for(j=0;j<(kx * ny);j++) array[j]=(float)arrayen[j];
  ...
  for (j = 1; j <= i__1; j += 2) {
    ...
    for (i__ = 2; i__ <= i__2; ++i__) {
      ...
      fredgo_do(&array[array_offset], &mn);
      /* L90: */
    }
...
/* ----------------------------------------------------------------------- */
/* Subroutine */ int fredgo_do(float *array,int * mn)
{
  /* System generated locals */
  float r__1;
