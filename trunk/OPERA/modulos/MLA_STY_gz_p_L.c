...
double Amoe_Funk_STY_gz_p_L_main(int n, double *x, double *y, double *p);
double Amoe_Funk_STY_gz_p_L_norm(int n, double *x, double *y, double *p);
double Amoe_Funk_STY_gz_p_L_prior(int n, double *x, double *y, double *p);
double Amoe_Funk_STY_gz_p_L_conf(int n, double *x, double *y, double *p);
double Funk2_int_STY_gz_p_L(double fluxreal);
double Funk1_intlumlog_STY_gz_p_L(double x);
void   EmpiricalCovars_STY_gz_p_L(int n,double *flux, double *z, double *errz, double *par, double *sigpar,double flim, struct cosmo_param cosmo,struct Schlf_L *lf);

void   ComputeNorma_STY_gz_p_L(int n, double flim, double strrad, double zlow, double zup, struct Schlf_L *lf);

struct Schlf_L *lf_STY_gz_p_L;
double *zsig_STY_gz_p_L;
struct cosmo_param *co_STY_gz_p_L;
double fluxl_STY_gz_p_L;
double z_STY_gz_p_L;
double flux_STY_gz_p_L;
double errz_STY_gz_p_L;
...
double MLmax;
double xf,Tf;
double zlow_STY_gz_p_L,zup_STY_gz_p_L,strrad_STY_gz_p_L;
...
int  MLA_STY_gz_p_L(int n,double *flux,double *z,double *errz, double flim, double strrad, double zlow, double zup, struct cosmo_param cosmo,struct Schlf_L *lf) {
  ...
  zsig_STY_gz_p_L=errz;
  ndata=n;
  co_STY_gz_p_L=&cosmo;
  fluxl_STY_gz_p_L=flim;
  strrad_STY_gz_p_L=strrad;
  zlow_STY_gz_p_L=zlow;
  zup_STY_gz_p_L=zup;
  ...
  VVmax_L(n,flux,z,flim,strrad,zlow,zup,cosmo,&lfvvmax);
/*   cpgopen("?"); */
...
/*   cpgclos(); */
  ...
  while(iter_amo==0) { 
    ...
    iter_amo=Amoeba_d(n,flux,z,3,par,sigpar,FTOL,MAXITER,Amoe_Funk_STY_gz_p_L_main);  
  ...
  MLmax=Amoe_Funk_STY_gz_p_L_main(n,flux,z,par);
  ...
  if(TRYEMPIRICAL) {
    ...
    conflim=exp(-.5/10.);
    EmpiricalCovars_STY_gz_p_L(n,flux,z,errz,par,sigpar,flim,cosmo,lf); 
...
/*   ComputeNorma_STY_gz_p_L(n, flim,  strrad, zlow,  zup, lf); */
...
/*   double scale; */
  ...
  for(i=0;i<ndata;i++) {
    z_STY_gz_p_L=y[i];
    flux_STY_gz_p_L=x[i];
    errz_STY_gz_p_L=zsig_STY_gz_p_L[i];
    Lumi=Lum(y[i],x[i],*co_STY_gz_p_L);
    ...
    if(xmin>=100) {
      ...
      if(fluxl_STY_gz_p_L-4*errz_STY_gz_p_L<0) { 
	x1=log(fluxl_STY_gz_p_L/30);  /* Con esto, solo nos perdemos el 10% del principio */
	probabajo=gaussintleg_d(Funk1_intlumlog_STY_gz_p_L,x1,x2,npb);
      }
      else   {
	x1=log(fluxl_STY_gz_p_L+4*errz_STY_gz_p_L);
	probabajo=gaussintleg_d(Funk1_intlumlog_STY_gz_p_L,x1,x2,npb);
	x1=log(fluxl_STY_gz_p_L-4*errz_STY_gz_p_L);
	x2=log(fluxl_STY_gz_p_L+4*errz_STY_gz_p_L);
	probabajo+=gaussintleg_d(Funk1_intlumlog_STY_gz_p_L,x1,x2,npb);
      }
      z_STY_gz_p_L=y[i];
      flux_STY_gz_p_L=x[i];
      errz_STY_gz_p_L=zsig_STY_gz_p_L[i];
      x1=zlow_STY_gz_p_L; 
      if(zlow_STY_gz_p_L<(z_STY_gz_p_L-6*errz_STY_gz_p_L)) x1=z_STY_gz_p_L-6*errz_STY_gz_p_L; 
      x2=zup_STY_gz_p_L; 
      if(zup_STY_gz_p_L >(z_STY_gz_p_L+6*errz_STY_gz_p_L)) x2=z_STY_gz_p_L+6*errz_STY_gz_p_L; 
      probarriba=gaussintleg_d(Funk2_int_STY_gz_p_L,x1,x2,npa); 
/*       intsch=lfamo.phistar*(intsup-incom(1+lfamo.alfa,xmin)); */
/*       printf(" El de sche %g y el nuevo %g    con flux %g err %g  tanto %f\n",Schechter_L(Lumi,lfamo)+ log(dLumdflux(y[i],*co_STY_gz_p_L)),log(probarriba),flux_STY_gz_p_L,errflux_STY_gz_p_L,errflux_STY_gz_p_L/flux_STY_gz_p_L);  */
...
/*   Ntot=Int_sch_L(lfamo,zlow_STY_gz_p_L,zup_STY_gz_p_L,fluxl_STY_gz_p_L,*co_STY_gz_p_L)*strrad_STY_gz_p_L/4./M_PI; */
...
/*   printf(" MLM %g conf %f   %f %f\n",MLmax,conflim,fabs(Amoe_Funk_STY_gz_p_L_main(n,x,y,p)-(MLmax-log(conflim))),Amoe_Funk_STY_gz_p_L_main(n,x,y,p)); */
   
  return(fabs(Amoe_Funk_STY_gz_p_L_main(n,x,y,p)-(MLmax-log(conflim)))); 
}
...
/*   printf(" antes for \n"); */
  ...
  for(i=0;i<nconfl;i++) {
    ...
    iter_c=Amoeba_d(n,flux,z,3,parconf,sigparconf,FTOL2,MAXITER2,Amoe_Funk_STY_gz_p_L_conf);
    ...
    if(iter_c==0 && Amoe_Funk_STY_gz_p_L_conf(n,flux,z,parconf)>FTOL2 ) {
      printf("\b.\b");
      fflush(NULL);
      i--;
    }
...
void   ComputeNorma_STY_gz_p_L(int n, double flim, double strrad, double zlow, double zup, struct Schlf_L *lf) {
  ...
  for(i=0;i<NBSNORMA;i++) {
    ...
    lfbs[i].phistar=1;
    Ntot[i]=Int_sch_L(lfbs[i],zlow,zup,flim,*co_STY_gz_p_L)*strrad/4./pi;
    if(VERBOSE) {
      printf(" %d Ntot %g\n",i,Ntot[i]);
  ...
  lf->phistar=1;
  Ntot_mean=Int_sch_L(*lf,zlow,zup,flim,*co_STY_gz_p_L)*strrad/4./pi;
...
double Funk1_intlumlog_STY_gz_p_L(double logfluxobs) {
...
/*   double scale,offset; */
  ...
  flux_STY_gz_p_L=fluxobs;
  x1=fluxl_STY_gz_p_L; 
  if(fluxl_STY_gz_p_L<flux_STY_gz_p_L-6*errz_STY_gz_p_L) x1=flux_STY_gz_p_L-6*errz_STY_gz_p_L; 
  x2=flux_STY_gz_p_L+6*errz_STY_gz_p_L; 
  if(fluxl_STY_gz_p_L>flux_STY_gz_p_L+5*errz_STY_gz_p_L) x2=x1+6*errz_STY_gz_p_L;
  firstint=gaussintleg_d(Funk2_int_STY_gz_p_L,x1,x2,npa); 
/*   printf(" Segunda integral %g\n",firstint); */
/*   printf(" El bueno %g y el sch %g lum %g\n",log(firstint),Schechter_L(Lumi,*lf_STY_gz_p_L),lumlog);  */
/*   printf(" Sale con %g\n",firstint*Lumi); */
  return(firstint*fluxobs); /* Este producto es para tener en cuenta que hacemos la integral en log(flux) */
}

double Funk2_int_STY_gz_p_L(double zreal) {
  double Lumi;
  double logfacLF,logfacerr,logfacder,logfacrho;
  if(flux_STY_gz_p_L<fluxl_STY_gz_p_L) return(0);
  if(zreal < zlow_STY_gz_p_L )return(0);
  if(zreal > zup_STY_gz_p_L )return(0);
  else {
    Lumi=Lum(zreal,flux_STY_gz_p_L,*co_STY_gz_p_L);
    logfacLF = Schechter_L(Lumi,*lf_STY_gz_p_L);
    logfacerr= log(gaussian(zreal,z_STY_gz_p_L,errz_STY_gz_p_L));
    logfacrho= log(Sch_rhoz_L(zreal,*lf_STY_gz_p_L,zlow_STY_gz_p_L,zup_STY_gz_p_L,fluxl_STY_gz_p_L,*co_STY_gz_p_L));
    /*     facder= dLumdflux(z_STY_gz_p_L,*co_STY_gz_p_L); */  
    /* Este de aqui arriba lo quito porque tardaria mas tiempo de computacion 
       Se hace mas deprisa como aqui abajo */
    logfacder= log(Lumi/flux_STY_gz_p_L);
...
/*     printf(" Y aqui %g con z %g flux %g err %g Lstar %g \n",log(Lumi),z_STY_gz_p_L,flux_STY_gz_p_L,errflux_STY_gz_p_L,log((*lf_STY_gz_p_L).Lstar));     */
/*     printf(" fluxreal %g/%g Lum %g Lstar %g val %g sch %g\n",log(fluxreal),log(fluxl_STY_gz_p_L),log(Lumi),log((*lf_STY_gz_p_L).Lstar),exp(Schechter_L(Lumi,*lf_STY_gz_p_L))*gaussian(flux_STY_gz_p_L,fluxreal,errflux_STY_gz_p_L),exp(Schechter_L(Lumi,*lf_STY_gz_p_L)));   */
    return(exp(logfacLF+logfacerr+logfacder+logfacrho));
  }
}
