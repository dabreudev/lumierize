...
double Amoe_Funk_STY_p_L_test_main(int n, double *x, double *y, double *p);
double Amoe_Funk_STY_p_L_test_norm(int n, double *x, double *y, double *p);
double Amoe_Funk_STY_p_L_test_prior(int n, double *x, double *y, double *p);
double Amoe_Funk_STY_p_L_test_conf(int n, double *x, double *y, double *p);
double Funk2_int_STY_p_L_test(double x);
double Funk1_norm_STY_p_L_test(double x);
void   EmpiricalCovars_STY_p_L_test(int n,double *flux,double *z,double *par, double *sigpar,double flim, struct cosmo_param cosmo,struct Schlf_L *lf);

void   ComputeNorma_STY_p_L_test(int n, double flim, double strrad, double zlow, double zup, struct Schlf_L *lf);
double *zsig;
...
int  MLA_STY_p_L_test(int n,double *flux,double *z,double flim, double strrad, double zlow, double zup, struct cosmo_param cosmo,struct Schlf_L *lf) {
...
/*   cpgclos(); */
  ...
  while(iter_amo==0) { 
    ...
    iter_amo=Amoeba_d(n,flux,z,3,par,sigpar,FTOL,MAXITER,Amoe_Funk_STY_p_L_test_main);  
  ...
  MLmax=Amoe_Funk_STY_p_L_test_main(n,flux,z,par);
  ...
  if(TRYEMPIRICAL) {
    ...
    conflim=exp(-.5/10.);
    EmpiricalCovars_STY_p_L_test(n,flux,z,par,sigpar,flim,cosmo,lf); 
...
/*     nemp_f++; */
/*     conflim=exp(-.5*16.);     */
/*     EmpiricalCovars_STY_p_L_test(n,x,errx,k,xk,Pk,sigpar,covar);  */
/*     nemp_f++; */
/*     conflim=exp(-.5*9.);     */
/*     EmpiricalCovars_STY_p_L_test(n,x,errx,k,xk,Pk,sigpar,covar);  */
/*     nemp_f++; */
...
/*   ComputeNorma_STY_p_L_test(n, flim,  strrad, zlow,  zup, lf); */
...
/*       logL-= log(Schechter_M(Mabs,lfamo_M)/intsch); */
      logL-= Schechter_L(Lumi,lfamo) + log(dLumdflux(y[i],*co)) - log(intsch);
...
/*   printf(" MLM %g conf %f   %f %f\n",MLmax,conflim,fabs(Amoe_Funk_STY_p_L_test_main(n,x,y,p)-(MLmax-log(conflim))),Amoe_Funk_STY_p_L_test_main(n,x,y,p)); */
   
  return(fabs(Amoe_Funk_STY_p_L_test_main(n,x,y,p)-(MLmax-log(conflim)))); 
}
...
/*   printf(" antes for \n"); */
  ...
  for(i=0;i<nconfl;i++) {
    ...
    iter_c=Amoeba_d(n,flux,z,3,parconf,sigparconf,FTOL2,MAXITER2,Amoe_Funk_STY_p_L_test_conf);
    ...
    if(iter_c==0 && Amoe_Funk_STY_p_L_test_conf(n,flux,z,parconf)>FTOL2 ) {
      printf("\b.\b");
      fflush(NULL);
      i--;
    }
