...
/* Por ahora es solo una copia de MLA_STY_p_M */
...
double Amoe_Funk_STY_p_M_main_wC(int n, double *x, double *y, double *p);
...
void   EmpiricalCovars_STY_p_M_wC(int n,double *magn,double *z,double *par, double *sigpar,double mlim, struct cosmo_param cosmo,struct Schlf_M *lf);
void   NumericalDerivCovars_STY_p_M_wC(int n,double *magn,double *z,double *par, double *sigpar,double mlim, struct cosmo_param cosmo,struct Schlf_M *lf);
...
//  _magDistn_STY_p_M_wC=malloc(n*sizeof(double));
//  _magDistn_STY_p_M_wC=malloc(n*sizeof(double));
//  for (i=0;i<=n;i++)
//  {
//     _magDistn_STY_p_M_wC[i]=magDistn[i];
//     _magSeln_STY_p_M_wC[i]=magSeln[i];
//  }
  _color_mean=color_mean;
  _color_stddev=color_stddev;
  ...
  iter_amo=Amoeba_d(n,magDistn,z,3,par,sigpar,FTOL,MAXITER,Amoe_Funk_STY_p_M_main_wC);
/*   printf(" FINAL par0 %.15g par1 %.15g \n",par[0],par[1]); */

  _MLmax_STY_p_M=Amoe_Funk_STY_p_M_main_wC(n,magDistn,z,par);
  ...
  if(TRYEMPIRICAL) {
    ...
    EmpiricalCovars_STY_p_M_wC(n,magDistn,z,par,sigpar,mlim,cosmo,lf); 
...
/*    printf("Dentro del trynumerical\n"); */
    NumericalDerivCovars_STY_p_M_wC(n,magDistn,z,par,sigpar,mlim,cosmo,lf); 
    if(DEBUG) printf(" sale \n");
...
double Amoe_Funk_STY_p_M_main_wC(int n, double *x, double *y, double *p)
{
  ...
  for(i=0;i<_ndata_STY_p_M;i++)
  {
    ...
    */
    ...
    logColor=log(gaussian(colori, _color_mean, _color_stddev));
  ...
  Ntot=Int_sch_M_wC(lfamo,_zlow_STY_p_M,_zup_STY_p_M,_color_mean,_color_stddev,_mlim_STY_p_M,*_cosmo_STY_p_M)*_strrad_STY_p_M/4./M_PI;
...
double Amoe_Funk_STY_p_M_main_gsl(double x, void *params)
{
  ...
  value=Amoe_Funk_STY_p_M_main_wC(nData, magn, z, lf_param);
