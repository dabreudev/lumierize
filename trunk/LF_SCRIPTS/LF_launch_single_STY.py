...
# TITLE: Script para generar calcular la LF con STY 
# CREATED: 20080324 dabreu
# Este script lanza el cálculo de la LF con STY
# colores
#
# MODIFIED: 20080422 dabreu
# LockFiles structure modified
# 
# Uso: LF_launch_single_STY.py -p simulationParameterFile numberOfIter
#

"""Script para calcualr la LF con STY"""
...
def main():
    ...
    def signal_handler(signal, frame):
        ...
        output.write("Recibí un SIGTERM (habló launch_single_STY)\n")
        try:
    ...
    for numObjects in nobjects:
        ...
        lfDir=simulationsDir+'/ComputedLF/STY/'+simulationName
        if not(os.access(lfDir,os.F_OK)):
            os.makedirs(lfDir)
            if verbose: output.write("Creating "+lfDir+"\n")
        computedSTYFile=lfDir+str(simulationNumber)+".lf"

        # done y lock files
        doneDir=simulationsDir+"/LockFiles/STY/"+simulationName
	...
	comando+=" "+computedSTYFile
        ...
        if doneFile:
            ...
            lockFileName=lockSTYFileName # para el capturador de sigterm
            global PID
            PID=False
            # mandamos el catálogo
            estatusSTY=secureCompute('STY',doneSTYFileName,lockSTYFileName,comando,computedSTYFile)
            if estatusSTY==1:
            ...
            if verbose: output.write("Ya está hecho STY para nobjects ="+str(numObjects)+"\n")
            continue
        else:
            if verbose: output.write("No se puede hacer STY porque no está el catálogo.\n")

    if verbose: output.write("Iter ="+str(simulationNumber)+"\n")

    # Cuando termina el bucle en número de galaxias
    if verbose: output.write("\nLF_launch_single_STY.py termina aquí\n")
    if verbose: output.write("\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n")
    output.close()
    sys.exit(0)
