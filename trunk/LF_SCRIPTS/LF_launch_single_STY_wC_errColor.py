...
# TITLE: Script para generar calcular la LF con STY_wC_errColor
# CREATED: 20080331 dabreu
# Este script lanza el cálculo de la LF con STY_wC_errColor
#
# MODIFIED: 20080425 dabreu
# Lockfiles structure modified.
#
# Uso: LF_launch_single_STY_wC_errColor.py -p simulationParamFile numberOfIter
#

"""Script para calcular la LF con STY_wC_errColor"""
...
def main():
    ...
    def signal_handler(signal, frame):
        ...
        output.write("Recibí un SIGTERM (habló launch_single_STY_wC_errColor)\n")
        try:
    ...
    for numObjects in nobjects:
        ...
        lfDir=simulationsDir+'/ComputedLF/STY_wC_errColor/'+simulationName
        if not(os.access(lfDir,os.F_OK)):
            os.makedirs(lfDir)
            if verbose: output.write("Creating "+lfDir+"\n")
        computedSTY_wC_errColorFile=lfDir+str(simulationNumber)+".lf"

        # done y lock files
        doneDir=simulationsDir+"/LockFiles/STY_wC_errColor/"+simulationName
	...
	comando+=" "+computedSTY_wC_errColorFile
        ...
        lockFileName=lockSTY_wC_errColorFileName # para el capturador de sigterm
        global PID
        PID=False

        if doneFile:
            # mandamos el catálogo
            estatusSTY_wC_errColor=secureCompute('STY_wC_errColor',doneSTY_wC_errColorFileName,lockSTY_wC_errColorFileName,comando,computedSTY_wC_errColorFile)
            if estatusSTY_wC_errColor==1:
                if verbose: output.write("Problemas con STY_wC_errColor\n")
	        continue # seguimos con otro
            else:
                if verbose: output.write("Todo correcto con STY_wC_errColor\n")
	 
            if verbose: output.write("Ya está hecho STY_wC_errColor para nobjects ="+str(numObjects)+"\n")
            continue
        else:
            if verbose: output.write("No se puede hacer STY_wC_errColor porque no está el catálogo.\n")

    if verbose: output.write("Iter ="+str(simulationNumber)+"\n")

    # Cuando termina el bucle en número de galaxias
    if verbose: output.write("\nLF_launch_single_STY_wC_errColor.py termina aquí\n")
    if verbose: output.write("\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n")
    output.close()
    sys.exit(0)
