...
# TITLE: Script para generar un catálogo con LumFunc_GOYA 
# CREATED: 20080322 dabreu
# Este script lanza la generación de un catálogo con LumFunc_GOYA utilizando
# colores
#
# MODIFIED: 20080339 dabreu
# To change commands by subprocess
#
# MODIFIED: 20080417 dabreu
# To change LockFiles structure
# 
# Uso: LF_launch_single_catalog.py -p simulationParameterFile numberOfIter
#

"""Script para generar un catálogo con LumFunc_GOYA"""
...
def main():
    ...
    def signal_handler(signal, frame):
        ...
        output.write("Recibí un SIGTERM (habló launch_single_catalog)\n")
        try:
    ...
    for numObjects in nobjects:
        ...
        if not(os.access(catDir,os.F_OK)):
            os.makedirs(catDir)
            if verbose: output.write("Creating "+catDir+"\n")
   
        simulatedCatalogFile=catDir+str(simulationNumber)+".cat"

        # done y lock files
        doneDir=simulationsDir+"/LockFiles/Cat/"+simulationName
	...
	comandoCat+=" "+str(zderr)+" "+str(mlim)+" "
        ...
        comandoCat+=str(area)+" "+simulatedCatalogFile

        lockFileName=lockCatFileName # para el capturador de sigterm
        global PID
        PID=False
        ...
        if verbose: output.write("Ya está hecho el catálogo para nobjects ="+str(numObjects)+"\n")
        continue

    if verbose: output.write("Iter ="+str(simulationNumber)+"\n")

    # Cuando termina el bucle en número de galaxias
    if verbose: output.write("\nLF_launch_single_catalog.py termina aquí\n")
    if verbose: output.write("\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n")
    output.close()
    sys.exit(0)
