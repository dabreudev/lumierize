...
# TITLE: Script para generar calcular la LF con VVmax 
# CREATED: 20080324 dabreu
# Este script lanza el cálculo de la LF con VVmax
# colores
# 
# MODIFIED: 20080422 dabreu
# LockFiles structure modified
#
# Uso: LF_launch_single_VVmax.py -p simulationParameterFile numberOfIter
#

"""Script para calcualr la LF con VVmax"""
...
def main():
    ...
    def signal_handler(signal, frame):
        ...
        output.write("Recibí un SIGTERM (habló launch_single_VVmax)\n")
        try:
    ...
    for numObjects in nobjects:
        ...
        lfDir=simulationsDir+'/ComputedLF/VVmax/'+simulationName
        if not(os.access(lfDir,os.F_OK)):
            os.makedirs(lfDir)
            if verbose: output.write("Creating "+lfDir+"\n")
        computedVVmaxFile=lfDir+str(simulationNumber)+".lf"
        fullDir=simulationsDir+'/VVmaxData/'+simulationName
        if not(os.access(fullDir,os.F_OK)):
            os.makedirs(fullDir)
            if verbose: output.write("Creating "+fullDir+"\n")
        fullVVmaxFile=fullDir+str(simulationNumber)+".cat"

        # done y lock files
        doneDir=simulationsDir+"/LockFiles/VVmax/"+simulationName
        ...
        doneVVmaxFileName=doneDir+str(simulationNumber)+".done"
        lockVVmaxFileName=doneDir+str(simulationNumber)+".lock"

        comando ="LF_computeVVmax_Mag.csh "+simulatedCatalogFile+" "
        comando+=str(zlow)+" "+str(zup)+" "+str(area)
	comando+=" "+str(mlim)+" "+computedVVmaxFile+" "+fullVVmaxFile

        doneFile=0
        doneFile=os.access(doneCatFileName,os.F_OK)

        lockFileName=lockVVmaxFileName # para el capturador de sigterm
        global PID
        PID=False

        if doneFile:
            # mandamos el catálogo
            estatusVVmax=secureCompute('VVmax',doneVVmaxFileName,lockVVmaxFileName,comando,computedVVmaxFile)
            if estatusVVmax==1:
            ...
            if verbose: output.write("Ya está hecho VVmax para nobjects ="+str(numObjects)+"\n")
            continue
        else:
            if verbose: output.write("No se puede hacer VVmax porque no está el catálogo.\n")

    if verbose: output.write("Iter ="+str(simulationNumber)+"\n")

    # Cuando termina el bucle en número de galaxias
    if verbose: output.write("\nLF_launch_single_VVmax.py termina aquí\n")
    if verbose: output.write("\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n")
    output.close()
    sys.exit(0)
